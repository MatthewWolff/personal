...
dat <- melt(dat, id=names(dat)[!(names(dat) %in% c("Complete","Partial", "Response.End", "Durable"))])
...
# Correctly formatted graph.
# ggplot(dat, aes(Patient.ID, Therapy.Length)) +  geom_bar(stat="identity", aes(fill=factor(Disease.Stage)), width=0.7) + coord_flip() +theme_bw() +
# theme(panel.grid.minor=element_blank(),panel.grid.major=element_blank(),
#       axis.text.y=element_blank(),axis.ticks.y=element_blank())
